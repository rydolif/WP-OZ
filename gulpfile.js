...
gulp.task('styles', function() {
	return gulp.src('OZ/assets/sass/**/*.sass')
	.pipe(sass({
...
gulp.task('img-responsive-1x', async function() {
	return gulp.src('OZ/assets/img/_src/**/*.{png,jpg,jpeg,webp,raw}')
		.pipe(newer('OZ/assets/img/@1x'))
		.pipe(responsive({
			'**/*': { width: '50%', quality: quality }
		})).on('error', function (e) { console.log(e) })
		.pipe(rename(function (path) {path.extname = path.extname.replace('jpeg', 'jpg')}))
		.pipe(gulp.dest('OZ/assets/img/@1x'))
});
// Produce @2x images
gulp.task('img-responsive-2x', async function() {
	return gulp.src('OZ/assets/img/_src/**/*.{png,jpg,jpeg,webp,raw}')
		.pipe(newer('OZ/assets/img/@2x'))
		.pipe(responsive({
			'**/*': { width: '100%', quality: quality }
		})).on('error', function (e) { console.log(e) })
		.pipe(rename(function (path) {path.extname = path.extname.replace('jpeg', 'jpg')}))
		.pipe(gulp.dest('OZ/assets/img/@2x'))
});
gulp.task('img', gulp.series('img-responsive-1x', 'img-responsive-2x', bsReload));

// Clean @*x IMG's
gulp.task('cleanimg', function() {
	return del(['OZ/assets/img/@*'], { force: true })
});

// Code & Reload
gulp.task('code', function() {
	return gulp.src('OZ/assets/**/*.php')
	.pipe(browserSync.reload({ stream: true }))
});

// Deploy
gulp.task('rsync', function() {
	return gulp.src('OZ/assets/')
	.pipe(rsync({
		root: 'OZ/assets/',
...
gulp.task('watch', function() {
	gulp.watch('OZ/assets/sass/**/*.sass', gulp.parallel('styles'));
	gulp.watch(['OZ/assets/js/common.js'], gulp.parallel('scripts'));
	gulp.watch('OZ/assets/*.php', gulp.parallel('code'));
	gulp.watch('OZ/assets/img/_src/**/*', gulp.parallel('img'));
});
